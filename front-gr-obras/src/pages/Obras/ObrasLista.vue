...
async function onRequest(props) {
  ...
  const { data, total = 0 } = await ObrasService.getData({
...
async function save() {
  obrasformRef.value.loading = true;

  try {
    await ObrasService.save(obrasformRef.value.form);
    obrasformRef.value.loading = false;
    obrasformRef.value.show = false;
    tableRef.value.requestServerInteraction();
    $q.notify({
          type: 'positive',
