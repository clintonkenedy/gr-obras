...
import AvancesService from "src/services/AvancesService";
import AvancesForm from "./AvancesForm.vue";
...
const columns = [
  ...
  },
  {
    ...
    label: "Obra",
    aling: "center",
    field: (row) => row.obra_id,
    sortable: false,
  },
];

const tableRef = ref();
const avancesformRef = ref();
const rows = ref([]);
const filter = ref("");
...
async function save() {
  avancesformRef.value.loading = true;

  try {
    await AvancesService.save(avancesformRef.value.form);
    avancesformRef.value.loading = false;
